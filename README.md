...
- [Onyx](https://onyx-actions.onrender.com) - Signed security verdict layer for agent payments: "is it safe to send?" as one x402 call. Pre-payment recipient firewall, as-deployed contract audit (proxy/self-destruct aware), ERC-8004 agent reputation, and pre-sign drain gates for token approvals, raw transactions, and EIP-712 signatures (Permit/Permit2/Seaport phishing patterns). Every verdict is Ed25519-signed (open OA-1 attestation envelope) and free to verify; a measured verdict-to-outcome track record (BLOCK precision / ALLOW miss-rate) is published via the free `onyx_track_record` tool. $0.02-$0.50 USDC per call on Base. MCP at `/mcp/`, REST at `/v1/<tool>`, A2A AgentCard, x402 V2 discovery at `/.well-known/x402.json`. ([GitHub](https://github.com/dimitrilaouanis-tech/onyx-mcp)) ([OA-1 spec](https://onyx-actions.onrender.com/.well-known/onyx-attestation/v1)) ([30s demo](https://github.com/dimitrilaouanis-tech/onyx-mcp/blob/main/DEMO.md))
